class CartBaseContainer extends BaseContainer {
    ...
    calculateSubTotal() {
        return [...this.state.cartItems].map(cartItem => Object.assign(cartItem, { rate: this.getItemFromCartItem(cartItem.name).rate })).reduce((subTotal, cartItem) => subTotal + cartItem.quantity * cartItem.rate, 0);
    }
